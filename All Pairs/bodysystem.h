...
inline float3
scalevec(float3 &vector, float scalar) // vector=scalar multiplication
{
    float3 rt = vector;
    rt.x *= scalar;
    rt.y *= scalar;
...
inline float
normalize(float3 &vector) // vector normalization
{
...
inline float
dot(float3 v0, float3 v1)  // dot-product
{
    return v0.x*v1.x+v0.y*v1.y+v0.z*v1.z;
}

inline float3
cross(float3 v0, float3 v1) // cross-product
{
    float3 rt;
